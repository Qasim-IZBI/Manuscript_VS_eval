...
in Supplementary Sec.~8). We count training in optimiser steps rather than epochs,
...
Each model is trained at three generator sizes, Small~(${\sim}10$M),
Medium~(${\sim}50$M), and Large~(${\sim}100$M) parameters, by adjusting the
...
\textbf{Training data fraction:}
The training set is partitioned into three nested subsets comprising 25\%, 50\%, and
100\% of the 30 training WSIs, constructed via contiguous WSI ranges to avoid data leakage.
...
The same fixed test set of $N{=}5$ held-out WSIs is used for evaluation across all configurations.
...
over $T_{\text{encode}}{=}$200~steps, and $\mathcal{M}_B$ then
decodes it into the target domain via DDIM sampling over
$T_{\text{sample}}{=}$200~steps.
